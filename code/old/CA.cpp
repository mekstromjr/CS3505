...
void convertRuleSetNumberToRuleSetArray (int ruleSet, int ruleSetArr[]) {
    for(int i = 7; i >= 0; i--) {
        int factor = 1;
        for (int j = i; j > 0; j--) 
            factor *= 2;
...
void computeNextGeneration(int curGen[], int nextGen[], int arrSize, int ruleSet[]) {
    //Cells on each end stay unchanged
    nextGen[0] = curGen[0];
...
int main() {
    ...
    for(int i = 0; i < generations; i++) {
        ...
        //Swap the pointer between the curGen and nextGen arrays so that what 
